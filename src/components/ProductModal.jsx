...
export default function ProductModal({ product, onClose, onAddToCart }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Content */}
      <div className="bg-skc-card rounded-[2rem] shadow-2xl relative w-full max-w-2xl md:max-w-4xl max-h-[90vh] flex flex-col overflow-hidden z-10 animate-fade-in-up border border-skc-copper/10">

        {/* Header con botón cerrar */}
        <div className="absolute top-4 right-4 z-20">
          <button
            onClick={onClose}
            className="bg-skc-surface hover:bg-white text-gray-700 w-10 h-10 rounded-full flex items-center justify-center shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-skc-copper"
        ...
        <div className="overflow-y-auto p-0 md:flex flex-col md:flex-row">
          ...
          {/* Imagen de Producto */}
          <div className="relative w-full md:w-1/2 bg-skc-surface border-r border-b md:border-b-0 border-skc-copper/20 flex items-center justify-center p-6 min-h-[300px]">
             <img
                src={imageUrl}
                alt={product.name}
          ...
          <div className="p-6 md:p-6 w-full md:w-1/2 flex flex-col justify-center">
            ...
            <div className="flex flex-wrap gap-2 mb-2 md:mb-1">
              {product.presentation && (
                <span className="text-xs font-semibold text-gray-600 bg-skc-surface border border-skc-copper/10 px-3 py-1 rounded-full">
                  {product.presentation}
                </span>
              )}
              {product.category && (
